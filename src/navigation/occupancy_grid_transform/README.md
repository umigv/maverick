...
This package consumes the occupancy grid provided by CV, adds a border on the far edges, applies obstacle inflation, and republishes two grids suitable for planning. The grid origin is transformed from the incoming frame to the configured output frame using TF2.

## Grid conventions
The occupancy grid from CV is expected to match the convention of [`nav_msgs/OccupancyGrid`](https://docs.ros2.org/foxyapi/nav_msgs/msg/OccupancyGrid.html)
- Row major
...
- `mission_state` (`maverick_msgs/msg/MissionState`) - Mission state (latched); selects which inflation parameter set is applied
...
## Inflation
After the grid is converted and a border is added, obstacle inflation is applied. For each occupied cell, surrounding cells are inflated based on their distance:
