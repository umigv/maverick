...
### Step 1 — Ray casting
Each tick, the node shoots a fan of rays outward from the robot's current position. The fan spans `arc_angle_rad` (default 180°) centered on the robot's heading, with rays spaced `ray_interval_rad` (default ~1.25°) apart.
...
- **Alignment term**: how well this ray aligns with the current momentum angle. Rays pointing along momentum score close to 1; rays pointing sideways or backward score closer to `alignment_floor` (default 0.1). The sharpness of this penalty is controlled by `alignment_gain`.
