...
class AutonavMissionControlConfig:
    ...
    Attributes:
        ...
        hsv_enable_near_exit_radius_m: Distance (m) from a no_mans_land_exit waypoint within which HSV is
            re-enabled. Allows the robot to see lane markings as it approaches the zone exit.
    ...
    hsv_enable_near_exit_radius_m: float = 3.0
    ...
    def __post_init__(self) -> None:
        ...
        if self.hsv_enable_near_exit_radius_m < 0:
            raise ValueError("AutonavMissionControlConfig: hsv_enable_near_exit_radius_m must be >= 0")
