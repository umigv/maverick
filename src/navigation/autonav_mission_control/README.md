...
the final waypoint is reached, `mission_complete` is set.
...
Inside no man's land, `hsv_enabled` is normally `false` to suppress HSV-based lane detection (lane markings are absent 
or unreliable in this zone). The exception: when the robot comes within `hsv_enable_near_exit_radius_m` of the next exit
waypoint, `hsv_enabled` is re-enabled so the robot can see lane markings as it approaches the zone boundary.
...
| `hsv_enabled` | `bool` | HSV lane detection should be active |
| `mission_complete` | `bool` | Final waypoint has been reached |
| `current_waypoint` | `geometry_msgs/PointStamped` | Map-frame position of the current target waypoint |
...
State is driven by a compute-and-apply loop in `check_waypoint_advancement`. Each entry in the loop is:

```python
("attr_name", self.compute_x, on_change_callback_or_None),
```
...
**3. Add it to the computations list** in `check_waypoint_advancement`:
```python
("my_flag", self.compute_my_flag, None),
```

**4. Publish it** by adding the field to the `MissionState` message in `publish_state`.

### On-change callbacks
If you need a side effect when a field transitions (e.g. starting a timer, calling a service), pass a callback as the 
third element instead of `None`. The callback receives `(old_value, new_value)` and is called after logging but before 
...
# in the computations list:
("my_flag", self.compute_my_flag, self._on_my_flag_change),
```
...
| `hsv_enable_near_exit_radius_m` | `float` | `3.0` | Distance (m) from the next `no_mans_land_exit` waypoint within which HSV is re-enabled |
