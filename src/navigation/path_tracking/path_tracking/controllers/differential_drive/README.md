...
The `cos(θ_err)` factor on the cross-track term fades it out when the robot is facing away from the path, preventing over-correction. Forward speed is reduced proportionally to `cos(θ_err)` and further capped so that lateral drift (`v · sin(θ_err)`) never exceeds `max_lateral_speed_mps`.
