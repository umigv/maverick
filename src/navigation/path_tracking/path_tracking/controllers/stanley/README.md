...
Forward speed is capped before corners by a lateral-acceleration budget: the controller looks `curvature_lookahead_m` ahead, accumulates total heading change, estimates the turn radius, and reduces speed so that `v^2 / r <= max_lateral_accel_mps2`. This slows the robot *before* it reaches the corner rather than reacting to it.
