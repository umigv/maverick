...
class HSV:
    ...
    def load_hsv_values(self):
        ...
        if hsv_file.exists():
            with Path.open(hsv_file) as file:
    ...
    def get_barrels_yolo(self):
        ...
        # mimic barrel_boxes from YOLO and generate mask the same way
        if self.barrel_mode not in self.hsv_filters:
        ...
        for cnt in contours:
            if cv2.contourArea(cnt) > 200:
                ...
                current_box = [
                    ...
                    x_max / width,
                    y_max / height,
                ]  # these are normalized apparently
    ...
    def tune(self, filter_name: str, use_zed: bool = False) -> None:
        ...
        if use_zed:
            cv2.createTrackbar(
                "BRIGHTNESS",
                "control panel",
                self.hsv_filters["__ZED_SETTINGS__"]["BRIGHTNESS"],
                8,
            ...
            cv2.createTrackbar(
                "CONTRAST",
                "control panel",
                self.hsv_filters["__ZED_SETTINGS__"]["CONTRAST"],
                8,
            ...
            cv2.createTrackbar(
                "HUE",
                "control panel",
                self.hsv_filters["__ZED_SETTINGS__"]["HUE"],
                11,
            ...
            cv2.createTrackbar(
                "SATURATION",
                "control panel",
                self.hsv_filters["__ZED_SETTINGS__"]["SATURATION"],
                8,
            ...
            cv2.createTrackbar(
                "SHARPNESS",
                "control panel",
                self.hsv_filters["__ZED_SETTINGS__"]["SHARPNESS"],
                8,
            ...
            cv2.createTrackbar(
                "GAMMA",
                "control panel",
                self.hsv_filters["__ZED_SETTINGS__"]["GAMMA"],
                9,
        ...
        if use_zed:
            if not ZED_AVAILABLE:
                print(
                    "Warning: ZED SDK (pyzed) not installed. Falling back to OpenCV. Trackbars will still change ZED settings."
                )
                use_zed = False
            else:
                zed = sl.Camera()
