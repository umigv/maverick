...
class ObstructedPedestrianDetection(FunctionalTest):
        ...
        for result in results:
            ...
            for box, confidence, class_id in zip(boxes, confidences, class_ids, strict=True):
                ...
                if class_id == 0 and confidence > 0.7:
                    _height, width = self.image.shape[:2]
                    range_ = int(width / 100)
                    size_person = (px2 - px1) / width
                    cv2.rectangle(self.image, (px1, py1), (px2, py2), (0, 255, 0), 2)
                    cv2.waitKey(1)
                    if size_person > 0.12:
                        # print("person within range")
                        return True, (py2 + range_), px1
                    return False, (py2 + range_), px1
    ...
    def state_machine(self):
        ...
        if self.state == 1:
            ...
            pedestrian_detected, _py2, _px1 = self.detect_pedestrian()
