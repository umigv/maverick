...
class LeftTurn(FunctionalTest):
    def __init__(self, debug: bool = False) -> None:
        ...
        self.x: int = 0
        self.y: int = 0
        self.edge_white_x: int = 0
        self.edge_white_y: int = 0
        ...
        self.midpoint: int | None = None
        self.in_state_4 = False
        self.testing = True
        self.debug = debug

    def find_slope(self, cur_x: int, cur_y: int, edge_white_x: int, edge_white_y: int) -> tuple[int, int]:
    ...
    def is_local_0s(self, mask: cv2.typing.MatLike, x: int, y: int) -> bool:
        if (
            not self.in_bounds(x, y)
            or not self.in_bounds(x + 1, y)
            or not self.in_bounds(x, y + 1)
            or not self.in_bounds(x + 1, y + 1)
        ):
            return False
        return bool(mask[y, x] == 0 and mask[y, x + 1] == 0 and mask[y + 1, x] == 0 and mask[y + 1, x + 1] == 0)

    def find_center_of_lane(self) -> list[tuple[int, int]] | None:
    ...
    def state_2(self) -> None:
        ...
        self.centroid = (int(self.width // 2.5), 100)

    # If we see yellow dashed, align with turn lane (and check for cone)
    def state_3(self, best_cnt: cv2.typing.MatLike) -> None:
        if self.debug:
            print("state_3")
        ...
        if self.hsv_obj is not None and self.hsv_obj.barrel_boxes is not None:
            for segment in self.hsv_obj.barrel_boxes:
                ...
                if y_min * self.height > self.height // 2:
                    ...
                    if (
                        self.midpoint is not None
                        and self.midpoint > self.width // 4
                        and self.midpoint < (self.width - (self.width // 4))
                    ):
                        self.in_state_4 = True
                        self.centroid = (self.midpoint, self.height // 2)
                        return
                else:
                    self.midpoint = None
            ...
            if y is not None and x is not None:
                ...
                while (
                    self.in_bounds(x, y)
                    and x < self.width - self.diff_x
                    and y < self.height - self.diff_y
                    and self.is_local_0s(self.white_mask, x, y)
                ):
                    x -= self.diff_x  # * 2, run
    ...
    def state_machine(self) -> None:
        ...
        contours, _ = cv2.findContours(
            self.yellow_mask[:, : self.width // 2], cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
        min_area = 200
