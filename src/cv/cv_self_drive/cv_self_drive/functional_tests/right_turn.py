...
class RightTurn:
    ...
    def state_3(self, best_cnt):
        ...
        if invalid_points:  # white line is probably gone, so finish the state
            ...
            while (
                (curr_x > 0 and curr_x < self.width - diff_x)
                and (curr_y > 0 and curr_y < self.height - diff_y)
                and self.white_mask[curr_y, curr_x] == 0
            ):
                curr_x += diff_x
                curr_y += diff_y
    ...
    def state_4(self, yellow_cnts):
        ...
        white_cnts, _ = cv2.findContours(
            self.white_mask[: self.height // 2, :], cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
        )
    ...
    def state_machine(self):
        ...
        if (num_yellow_dashed == 0 or (best_cnt is None)) and not self.state_2_done:  # state 2
            self.state_2()
            return
        if not self.state_3_done:  # state 3
