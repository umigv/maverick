...
class CameraUI:
    ...
    def __init__(self, panel_size: int = 240) -> None:
    ...
    def render(self, grids: list[npt.NDArray], merged: npt.NDArray) -> bool:
        grid1 = grids[0] if len(grids) > 0 else np.zeros((10, 10))
        grid2 = grids[1] if len(grids) > 1 else np.zeros((10, 10))

        def make_panel(title: str, g: npt.NDArray, color: Scalar) -> npt.NDArray:
...
def cam_init(ser: int | None = None) -> sl.InitParameters:
...
def print_pos(ui_params: dict[str, dict[str, int]]) -> None:
