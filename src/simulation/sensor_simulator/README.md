...
- Applies OU multiplicative scale-factor drift to both linear and angular velocity proportional to speed so a stationary robot always reads near zero and odom drift only accumulates while moving
...
The VN-300 is modeled as a single sensor with one shared Kalman filter state per quantity, matching how the real hardware works — all four outputs share the same OU drift states, so `gps`, `imu`, `ins_vel`, and `odom` stay mutually consistent.
