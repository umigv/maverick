...
ROS 2 driver node for the VectorNav VN-300 IMU/INS. Publishes IMU, GNSS fix, body velocity, odometry, and status topics. At startup it reads TF transforms to configure sensor offsets on the device, then streams binary output packets at the configured rate.
...
| `imu_frame_id`                  | `str`      | -            | TF frame ID for the IMU origin                                                                         |
...
Live terminal display of INS and GNSS status, decoding the `ins_status`, `gnss_status`, and `gnss2_status` bitfields. Useful for monitoring startup progress and diagnosing fix issues.
